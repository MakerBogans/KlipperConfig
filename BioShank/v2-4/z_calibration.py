...
class CalibrationState:
    ...
    def _probe_on_site(self, endstop, site, check_probe=False):
        ...
        #self.gcode.respond_info("Pos5 %.3f " % (pos[2],))
        if pos[2]-self.helper.clearance <= self.helper.clearance:
            # no clearance, better to move up
            #self.helper._move([None, None, pos[2] + self.helper.clearance], self.helper.lift_speed)
            self.helper._move([None, None, self.helper.clearance], self.helper.lift_speed)
        # move to position
    ...
    def calibrate_z(self):
        ...
        #pos = self.toolhead.get_position()
        #self.gcode.respond_info("Pos3 %.3f " % (pos[2],))
        probe_site = self._add_probe_offset(self.helper.bed_site)
        #pos = self.toolhead.get_position()
        #self.gcode.respond_info("Pos4 %.3f " % (pos[2],))
